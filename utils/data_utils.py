...
def numpy_read_func(batch_path):
  ...
  for i in range(len(batch_path)):
    np_list.append(
        np.load(os.path.join(FLAGS.img_data_dir, batch_path[i].decode("utf-8")))
    )
...
def token_split_func(batch_token, max_length, cap=None):
  ...
  for i in range(FLAGS.batch_size):
    if b'_' in batch_token[i]:
      valid_ids = list(map(int, batch_token[i].decode("utf-8").split('_')))[:max_length]
